...
var scan = {
    ...
    init: function(){
        scan.APIrequest(drupalsites.sites[scan.tmp.site].api.test,startPage,scan.worker);
    },
    worker: function(body, page){
        ...
        for(var a = 0; a < body.contents.length; a++){
            ...
            if(found == 0){
                ...
                console.log("WARNING - NO CATEGORY FOUND ON THIS TEST NODEID: " + body.contents[a].nid);
                scan.tmp.errors++;
        ...
        if(page < scan.tmp.pages){ 
            console.log(" ");
            scan.APIrequest(drupalsites.sites[scan.tmp.site].api.test,page++,scan.worker);
        } else {
            ...
            if(site <= drupalsites.sites.length){
                ...
                scan.APIrequest(drupalsites.sites[scan.tmp.site].api.test,scan.tmp.pages,scan.worker);
            }
        }
