...
var scan = {
    ...
    init: function(){
        scan.APIrequest(drupalsites.sites[scan.tmp.site].api.subpage,startPage,scan.worker);
    },
    worker: function(body, page){
        ...
        for(var a = 0; a < body.contents.length; a++){
            ...
            if(found == 0){
                ...
                console.log("WARNING - NO CATEGORY FOUND ON THIS SUBPAGE NODEID: " + body.contents[a].nid);
                scan.tmp.errors++;
        ...
        if(page < scan.tmp.pages){ 
            console.log(" ");
            scan.APIrequest(drupalsites.sites[scan.tmp.site].api.subpage,page++,scan.worker);
        } else {
            //continue to next site
            //TODO: When last site is reached, end script.
            
            if(site <= drupalsites.sites.length){
                scan.tmp.pages = 0;
                scan.tmp.site = parseInt(scan.tmp.site) + 1;
                scan.APIrequest(drupalsites.sites[scan.tmp.site].api.subpage,scan.tmp.pages,scan.worker);
            }
        }
